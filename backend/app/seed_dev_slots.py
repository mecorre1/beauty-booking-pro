...
def _slots_for_iso_week(year: int, week: int) -> list[Slot]:
    """Fixed pattern per week: Mon (2), Tue (1), Thu (2) with mixed availability."""
    monday = date.fromisocalendar(year, week, 1)
    specs: list[tuple[int, int, int, int, int, bool]] = [
        (0, 9, 0, 9, 45, True),  # Mon
        (0, 11, 30, 12, 15, False),  # Mon
        (1, 10, 0, 11, 0, True),  # Tue
        (3, 14, 0, 15, 30, True),  # Thu
        (3, 16, 0, 16, 30, False),  # Thu
    ]
    rows: list[Slot] = []
    for day_off, sh, sm, eh, em, avail in specs:
